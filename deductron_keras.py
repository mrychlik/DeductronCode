...
Modified by Marek Rychlik (c) 2020. Allowed output size to be different from memory size.
...
class DeductronCell(Layer):
    ...
    # Arguments
        units: Positive integer, dimensionality of the memory space.
        output_size: Positive integer, dimensionality of the output space.
    ...
    def build(self, input_shape):
        ...
        if self.use_bias:
            ...
            self.output_bias = self.add_weight(shape=self.output_size,
                                        name='output_bias',
                                        initializer=bias_initializer,
                                        regularizer=self.bias_regularizer,
                                        constraint=self.bias_constraint)
...
#        return np.zeros([batch_size, cell.state_size], dtype = dtype)
